 ...
 * Approach of detachable threads
*/
...
{
    // Welcome Message
    printf("Starting the program! ---> Multiple Detached Threads Num: %d\n", THREADS_NUM);
    fflush(stdout);
   ...
   // start = clock(); //-> start of clock
    ...
    for (int i = 0; i < THREADS_NUM; i++)
    {
        ...
        // this is used to prevent race condition and .. read more about this
        int *threadStart = malloc(sizeof(int));
        *threadStart = startRow;

        // create detachable threads by making the attribute detachhed:
